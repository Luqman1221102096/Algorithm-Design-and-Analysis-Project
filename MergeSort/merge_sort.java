...
public class merge_sort {
  ...
  public static void main (String args[]) { 
    ...
    String line;
    String filePath = "MergeSort/testDataSet.csv";
  ...
  static void writeCsv(ArrayList<Integer> S, ArrayList<String> id){
    //Clears file
    try (BufferedWriter writer = new BufferedWriter(new FileWriter("merge_sort_"+ S.size()+ ".csv"))) {
            writer.write("");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    for (int i = 0; i < S.size(); i++){
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("merge_sort_"+ S.size()+ ".csv", true))) {
